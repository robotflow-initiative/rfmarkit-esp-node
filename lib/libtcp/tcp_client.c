...
void tcp_client_set_destination(tcp_client_t* client, const char* ip_addr, uint16_t port) {
...
esp_err_t tcp_client_create_socket(tcp_client_t* client) {
...
esp_err_t tcp_client_set_timeout(tcp_client_t* client, int timeout_s) {
...
void tcp_client_init(tcp_client_t* client, int timeout) {
    tcp_client_set_destination(client, client->address, client->port);
    tcp_client_create_socket(client);
    if (timeout > 0) {
        tcp_client_set_timeout(client, timeout);
    }
}
